...
class Optimiser:
    ...
    def create_optimisation_problem(
        ...
        for _t in self.time_slices:
            ...
            self.problem += (
                self.variables.battery_state_of_charge[_t]
                <= self.variables.battery_capacity
            )
    ...
    def dump_results(self, output_path: Path) -> None:
        ...
        if (
            ...
            results["battery_cost"] = (
                pl.value(self.variables.battery_capacity) * self.battery_capex
            )
            results["solar_cost"] = (
    ...
    def _define_objective_function(
        ...
        if self.optimisation_objective == OptimisationObjectives.MINIMISE_BATTERY_CAP:
            if battery_capex:
                logger.info(
                    "Battery capex has been provided as an input argument, however, the optimisation objective"
                    "is to minimise battery size. Therefore, this battery capex will be ignored."
                )
            if solar_capex:
                logger.info(
                    "Solar capex has been provided as an input argument, however, the optimisation objective"
